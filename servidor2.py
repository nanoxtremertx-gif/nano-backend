# servidor2.py (V10.16 - MODO BLINDADO Y DEBUG)
...
# Ajustamos la ruta absoluta para Docker
BASE_DIR = Path("/app")
...
def load_model_server(model_name):
    if model_name not in MODELS_CACHE:
        ...
        if not path.exists():
            ...
            # Intento desesperado: buscar en la raíz
            path = BASE_DIR / model_name
            if not path.exists():
                raise FileNotFoundError(f"Modelo {model_name} no existe en {MODELS_DIR} ni en {BASE_DIR}")
...
def reconstruct_and_degrade(crs_data_bytes: bytes, password: str = None) -> str:
    ...
    try:
        ...
        raise RuntimeError(f"Fallo técnico en reconstrucción IA: {str(e)}")

    if final_array is None: raise ValueError("Error interno: Imagen vacía tras reconstrucción.")
