# servidor2.py (V10.17 - FIX FINGERPRINT & ROBUST AUTH)
...
print(">>> INICIANDO SERVIDOR 2 (IA)...", flush=True)
...
try:
    ...
    TF_AVAILABLE = True
    print(f">>> TENSORFLOW CARGADO. VERSIÓN: {tf.__version__}", flush=True)
except ImportError:
    print("!!! ERROR CRÍTICO: TensorFlow no instalado.", file=sys.stderr)
except Exception as e:
    print(f"!!! ERROR CARGANDO TENSORFLOW: {e}", file=sys.stderr)
...
# --- HELPER: EXTRACCIÓN INTELIGENTE DE AUTOR ---
def get_author_fingerprint(data_dict):
    """Busca el ID del autor en múltiples campos posibles."""
...
def reconstruct_and_degrade(crs_data_bytes: bytes, password: str = None) -> str:
    ...
    # 1. Desencriptar y Obtener Autor
    final_data = crs_data
    ...
    # Usamos la función inteligente para buscar el autor en los datos desencriptados
    author_fingerprint = get_author_fingerprint(final_data)

    # 2. Reconstrucción Técnica
    ...
    try:
        if file_version.startswith("51."):
            ...
            original_shape, compressed_tiles = final_data["original_shape"], final_data["payload_list"]
            tile_size = final_data["tile_size"]
            ...
            core_seed, fidelity_seed = final_data["core_seed"], final_data["fidelity_seed"]
    ...
    # AQUÍ SE PONE EL FINGERPRINT CORREGIDO
...
def handle_preview_generation():
    ...
    try:
        ...
        print(f"ERROR IA: {e}", file=sys.stderr)
